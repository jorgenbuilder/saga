...
export default async function uploadAssets() {
    //@ts-ignore
    for (const file of await window.showOpenFilePicker({ multiple: true })) {
        console.log(file);
        ...
        const data = `data:image/${file.name.split('.').slice(-1)[0]};base64,${b64}`;
        await deck.uploadAsset(index, data);
    };
}
